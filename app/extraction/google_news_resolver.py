    ...
    def resolve(self, url: str) -> ResolutionResult:
        ...
        try:
            ...
            if isinstance(res, dict) and res.get("status") is True:
                ...
                if decoded_url and not self.is_google_news_url(decoded_url):
        ...
        try:
            with httpx.Client(timeout=self.timeout, follow_redirects=True, headers=headers) as client:
                ...
                if not self.is_google_news_url(final_url):
                ...
                if data_n_au and not self.is_google_news_url(data_n_au[0]):
                    logger.info("Successfully resolved Google News URL via HTML data attribute: %s -> %s", clean_url[:60], data_n_au[0])
                    return ResolutionResult(
                        original_url=clean_url,
                        resolved_url=data_n_au[0],
                        is_google_news=True,
                        success=True,
                ...
                non_google_hrefs = [h for h in hrefs if not self.is_google_news_url(h) and "google" not in urlparse(h).netloc]
                if non_google_hrefs:
