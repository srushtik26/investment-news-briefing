...
def run_pipeline(
    ...
    # =========================================================================
    is_weekend_resumed = restore_partial_weekend_state(ctx, data_dir)
    if is_weekend_resumed:
        log_exec('[WEEKEND_RECOVERY_RESUMED] Resumed from partial weekend state. Skipping baseline extraction & clustering.')
        if not ctx.india_reserve_pool or not ctx.intl_reserve_pool or not ctx.domestic_reserve_pool:
            discover_initial_reserves(ctx, max_domestic, max_india, max_international)
    else:
        log_exec("=" * 60)
